<div class="modal fade" id="showFaculty" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="Faculty" aria-hidden="true">
	<div class="modal-dialog modal-lg modal-dialog-scrollable">
		<div class="modal-content">
			...
			<div class="modal-body">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        {{ $faculty_show->faculty_id }}
                        <br>
                        <label>First Name: </label>
                        {{ $faculty_show->first_name }}
                        <br>
                        <label>Middle Name: </label>
                        {{ $faculty_show->middle_name }}
                        <br>
                        <label>Last Name: </label>
                        {{ $faculty_show->last_name }}
                        <br>
                        <label>Gender: </label>
                        {{ $faculty_show->gender }}
                        <br>
                        ...
                        <br>
                        <label>Address: </label>
                        {{ $faculty_show->address }}
                    </div>
                    <div class="col-md-6">
                        <label>Account Status: </label>
                        @isset ($faculty_show->user)
                        <span class="text-success">Active</span>
                        <br>
                        <label>Username #: </label>
                        {{ $faculty_show->user->user->username }}
                        <br>
                        <label>Email #: </label>
                        {{ $faculty_show->user->user->email }}
                        @else
                        ...
                        <form action="{{ route('users.store') }}" method="POST" autocomplete="off">
                            ...
                            <input type="hidden" name="user_id" value="{{ $faculty_show->id }}">
			...
			<div class="modal-footer">
				<div class="col">
					@if ($faculty_show->trashed())
                		@can('faculties.restore')
					    <a class="btn btn-default text-success" href="javascript:void(0)" onclick="restoreFromTable(this)" data-href="{{ route('faculties.restore', $faculty_show->id) }}"><i class="fad fa-download"></i> Restore</a>
						@endcan
					@else
						@can('faculties.destroy')
					    <a class="btn btn-default text-danger" href="javascript:void(0)" onclick="deleteFromTable(this)" data-href="{{ route('faculties.destroy', $faculty_show->id) }}"><i class="fad fa-trash-alt"></i> Delete</a>
						@endcan
					@endif
					@can('faculties.edit')
